...
This document explains the core concepts of **single-level paging** as used in early virtual memory systems (e.g., Atlas, IBM System/360 Model 67). Unlike modern multi-level schemes, early paging used a flat page table per process—simple, efficient, and foundational to all modern memory management.

> 💡 **Key Idea**: Virtual memory gives each process the illusion of a large, contiguous address space—even though its data may be scattered across physical RAM.
...
| **Page Table** | An array where each entry maps a virtual page → physical frame. Created per process. |
| **PTBR** (Page Table Base Register) | Hardware register holding the **physical address** of the current process’s page table. |
| **MMU** (Memory Management Unit) | Hardware that translates virtual → physical addresses using PTBR + page table. |
| **Page Fault** | CPU exception triggered when accessing an unmapped (invalid) virtual page. |

> 📌 **Note**: The **offset is part of the virtual address itself**—it is **not stored in PTBR**.
...
Virtual Address: [ 20-bit Page Number | 12-bit Offset ] └─── index into ────┘ └─ byte in page ─┘


**Translation Steps**:
1. MMU reads **PTBR** to locate the page table in physical RAM.
2. Uses **page number** to index into the page table → gets physical frame number.
3. Combines frame number + offset → final physical address.
